...
load profiling, that takes into account consumption patterns of residents. 
...
\cite{Csoknyai2019} analyse energy consumption patterns and intervention strategies in residential buildings.
Authors achieve this using "serious game approach" with combination of direct user feedback using smart meters. 
Application also provides advice, comparisons, savings, reduction goals, and monitoring.
...
Again using appliance load profiles to mitigate net energy flow as close to zero as possible i.e. zero-energy building.
...
This process is called vehicle-to-grid and in the process, consumers obtain a new name "prosumers", 
since they take part in generation of the produce \cite{Prosumer2016}.
It will be an essential part of European Unions plan to reach zero-energy buildings
and near-zero-energy buildings \cite{eu2021}. Directive was accepted in 2010 and was recast in 2021.
Plan is set to be realised in next decade.
\cite{EV2018} and \cite{EV2020} Implemented V2G to reduce net flow of energy.
...
"DR program is a voluntary PJM program that compensates end-use (retail) customers for reducing their electricity use (load) when requested by PJM during periods of high power prices, or when the reliability of the grid is threatened." \cite{DemandResponse2018}
...
\cite{energy_stealing_2018} use anomaly detection algorithms and load profiling to detect energy lost due to non-tehnical loses.
This occurs after smart-meter is exposed to cyberattack and its measuremnts are off. 

One other use case could be occupancy detection of buildings such as authors explore in \cite{occupancy2013}. Information about 
