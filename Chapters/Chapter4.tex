...
In general, each LP has its use-case already assigned in Table \ref{tab:use_cases}.
...
Secondly, using the same pattern, we will present per-appliance LPs.
Finally, we will present per-building per-appliance LPs.
Data for LPs in this chapter was used from building 2 from the REFIT dataset discussed in Section \ref{ssec:data}.
...
The daily LPs are the most commonly used LPs, as can be seen in Table \ref{tab:contributions}
Generally, they are the easiest to build since they do not need as much data as others do.
To build a decent LP one needs enough data. 
A sufficient amount of data is the amount that covers major events.
For a daily LP, a few weeks of data is enough, weekly LPs need a few months of data, monthly few months, and yearly few years.
And this is the main issue, there is rarely enough data to build such LPs.
Even then, usage patterns could change over a long period such as a decade.
Combining that with a smaller number of use cases for such LPs, reveals why such LPs were not looked into as much in Table \ref{tab:contributions}.

One more thing about time ranges that need to be mentioned are patterns that they present.
Daily LPs present daily usage and enable us to extract contextual events such as waking up, cooking, leisure time, etc.
...
The monthly LP has none of the above. It is not repetitive since each day of the month can be a different day of the week, and the period is too short to capture seasonal patterns.
Alternatively, it could be presented as a week in a month, but there is no significant usage pattern to be revealed.
The yearly LP on the other hand presents the seasonal effects on usage such as increased daylight and temperature. 

\section{Per-Building LPs}
\label{sec:per_building}
The section will be focused on per-building LPs.
Per-building LPs refer to representations where whole building usage is presented as a single LP.
...
According to Table \ref{tab:contributions} this is the most commonly used power LP.
Figures \ref{fig:SLPdaily2} and \ref{fig:SLPyearly2} present usage patterns on different time ranges. 
The two LPs, therefore present different contextual cases.
...
Previously, in Section \ref{sec:grid_managment}, it was mentioned that these kinds of LPs are the most applicable in grid management. 
...
This also means that it is one of the most universal LPs since it can be used in use-cases defined in Section \ref{sec:use-cases}
...
In the case where dwellers have different usage patterns during the weekends, two LPs would have to be developed.
...
By combining weekly wind forecasts, weekly cloud coverage, and user consumption LPs energy providers could notify users to turn on their appliances at peak usage times.
...
Roughly 5 years of data mean that it is possible to build a decent LP. 
...
The last group of LPs is a combination of per-building and per-appliance LPs.
...
This LP shows that the first thing in the morning used are a kettle and toaster, and with a delay of one hour, microwave and TV. 
...
\begin{figure}[H]
	...
	\includegraphics[]{../Figures/LPS/PHPA_LP_for_building.pdf}
	\label{fig:PHPA}
\end{figure}
...
To be able to process the LPs across many buildings a new LP, seen in Figure \ref{fig:BOA}, must be introduced.
...
The Bag of appliances was not shown on the Table of LPs \ref{tab:our_rd},
since it is a special case of the per-building per-appliance LP shown in Figure \ref{fig:PHPA}.

\section{Conclusion}

This chapter showed how some activation LPs from Table \ref{tab:contributions} present real-world data, analyzed the presentations and further elaborated on their use-cases.
...
Finally, we have shown how more detailed LPs \ref{fig:PHPA} could be used for practical applications such as elderly care. 