...
Data for profiles in this chapter was used from building 2 from the REFIT dataset.
Data was collected from 2013-09-18 to 2015-05-28.

\section{Time ranges}
\label{sec:time_range}
One important thing to mention is the use cases for different time ranges of load profiles.
...
In the case of activation load profiles, this is an elemental load profile, since all other activation profiles are built on top of it. 
This also means that it is one of the most universal profiles since it can be used in all previously defined use cases.
Additionally, it is possible to analyze the usage of a single appliance in great detail.
...
Looking at Figure \ref{fig:WM_daily}, we can detect a similar pattern as in per-building Section \ref{sec:per_building}. 
While the peaks are closer together, the pattern remains.
One thing to notice here is that the washing machine is used only throughout the day, this means that the household does not use the cheaper tariffs during the nighttime.
...
An optimal number of buckets is a number that presents the usage pattern clearly. 
...
the TV was at it's low. And when the usage of computers decreased, the usage of TV increased. 
...
This will be important later in Chapter \ref{Chapter7}, when we will be designing an elderly care system, that will be based on periodical user behavior.
...
Combining the Figures \ref{fig:solar_thermal_pumping_station} and \ref{fig:light} enables us to differentiate between the two. 
