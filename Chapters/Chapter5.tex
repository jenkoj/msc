...
We found that by calculating Euclidean distance between points on t-SNE map we could detect the presence of a routine, which in turn would allow us to detect anomalies. 
As mentioned, this will be explored in higher detail in the next Chapter \ref{chapter6}.
The advantage of this method is that we have used the main meter data, which significantly increases the applicability of this approach. 
The ease of installation of the system is essential for the successful adoption of the emerging technologies. 
...
In addition to the t-SNE, it would be interesting to examine other dimensionality reduction methods such as PCA or more advanced UMAP method. 
