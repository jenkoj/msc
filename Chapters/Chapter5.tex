...
Nonetheless, through analysis, we can find clusters with similar usage patterns.

It is possible to see that on the left there are mostly samples with very little activity, and on the right, we see samples with more activity.
We can observe a clear cluster of LPS from \textit{ukdale1} and \textit{ukdale2}.
Besides the cluster being clear, it contains a lot of samples as well.
From Section \ref{ssec:ds_eval} we know that \textit{ukdale1} and \textit{ukdale2} contain plenty of appliances which could point to the fact that these are LPs contain a lot of activators.
When observing the figure more precisely we can see that in the central part of the plot, LPs form smaller clusters of various patterns.
Few less obvious clusters were formed in the bottom section of the plot for buildings (looking from the left) \textit{refit2}, \textit{refit9}, \textit{refit5}, \textit{refit6}, \textit{refit4}, \textit{refit8}.
An important observation that can be made here is how close together the LPs are.
The closer together the samples are the more similar the consumption throughout every month is.
In other words, by measuring the distance between LPs of the same building, we could measure the strength of the routine that each household has.
This is an important observation that will be used in the next Chapter \ref{chapter6}.

Patterns in the central part of the plot have no real pattern that we can recognize.
The reason behind this could be that appliances such as fridges have contributed the majority of activations.
Since such appliances are not affected by the dweller, such LPs present random activations. 
That is why there are no clusters in the central part of the plot as can be observed in Figure \ref{fig:tsne_scatter_non_norm_all} above.

The same goes for the profiles in the far left part of the plot, where there are no clusters to be observed.
In this case, the reason is that these profiles do not contain enough information, which could be used to distinguish them.


Even though the LPs contained a non-normalized amount of activations, we were still able to identify interesting patterns through the t-SNE plot
