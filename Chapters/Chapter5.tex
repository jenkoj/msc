...
So in order to calculate the t-SNE for a set of data points, we first need to calculate the conditional probability.
This is calculated based on the Equation \ref{eq:pij} below.
The author Van der Mateen in his publication \cite{tsne2} states:
...
Where $x_i$ and $x_j$ are two data points. And $|x_i - x_j|$ is the Euclidean distance between the two.
In simple terms, euclidean distance is a difference in values.

The nominator in \ref{eq:pij} is equal to the similarity between two points normalized by the $2\sigma_i^2$.
The whole expression is run through $exp()$ function to keep the value within boundaries.

The denominator in \ref{eq:pij} does the very same thing, but it sums every for every pairwise similarity.
It guarantees that the similarity sum to 1 in the case of the same points. 

The output of the algorithm is a map of every data point $y_i$.
These points are low dimensional counterparts of $x_i$.
Usually, these data points contain comprehendible number of dimensions such as $\in \mathbb{R}^2$ or $\mathbb{R}^3$. 
Similarly as in equation \ref{eq:pij} we can now use low dimensional data points $y_i$ and $y_j$ to calculate probability $q_{i|j}$ 

\begin{equation}
\label{eq:gij}
q_{i|j} = \frac{(1+\lVert y_i - y_j \rVert^2)^{-1}}{\sum_{k \neq l} (1+\lVert y_k - y_l \rVert^2)^{-1}}
\end{equation}

$q_{ij}$ is again a conditional probability of low finding $y_i$ and $y_j$ near each other but for fewer dimensions.

Setting up a cost function, which tries to minimize the difference between $q_{ij}$ and $p_{ij}$ shout results in a low dimensional map where similar points should be near eachother.
...
The smaller the $C$ the closer together similar low dimensional data points to high dimensional pairs.
...
The similarity is achieved over many iterations where we use gradient descent to minimize the kullback-leibler divergence. 
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \node (input) at (0,0) {\includegraphics[width=4cm]{Figures/TSNE/DEMO/input_clean.pdf}};
    ...
    \end{tikzpicture}
    \caption{2D input figure transformed into 1D output figure using t-SNE}
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \draw[->, thick] (tsne1.east) -- (tsne2.west) node[midway, above] {100 iters};
        \draw[->, thick] (tsne2.east) -- (tsne3.west) node[midway, above] {200 iters};
    \end{tikzpicture}
    \caption{Iterations of t-SNE with arrows between subfigures}
