...
The third group of appliances are white goods with a compressor.
...
If we calculate entropy for each image and set some low threshold, it is possible to filter out these samples. 
Setting an entropy threshold of 0.5, we filter out around 5 \% of all samples. 
...
As we can see on figure \ref{fig:tsne_boa_scatter_refit8} the clusters are nicely formed
but are separated quite a lot. The problem with such comparison is, that it is best 
if all boulings would have the same appliance. Since this is never the case,
it adds the reason why they are seperated. 
