...
\section{Correlation of t-SNE Euclidean Distance and Routine Rate}
...
In the case of t-SNE, we have used normalized per-building LPs with very little preprocessing, whereas in elderly care algorithm, we have used per-building per-appliance LPs with extensive pre-processing to extract applicnes that could help us detect the routine.

\subsection{Discussion}
This connection is an important proof that confirms our statements from Chapter \ref{chapter5} that the dispersion of samples in the t-SNE plot is related to routine.
This also means that we can rely on t-SNE to evaluate the routine of a household.
While it may not be as precise as an elderly care algorithm, it is simple to use. 
...
It should also be made clear that their data will be handled according to GDPR regulations.
\subsection{Summary}
