...
Such distribution makes it hard to statistically analyze the results and makes it hard to draw meaningful conclusions.
However, this issue can be addressed by visually analyzing and interpreting the data.
In this case, is possible to see that a large portion (74 \%) of the samples fall in the interval above the mean, more specifically above the 80 \% routine rate.
...
If we assume that the average building in our dataset has altered routine through the weekend as can be seen in \ref{fig:ec_week},
...
We can observe that the mean test routine is 80.42 \%, which is slightly higher compared to results from the previous Table \ref{tab:ec_res_weekend}.
...
The last rows in Tables \ref{tab:ec_res_no_weekend} and \ref{tab:ec_res_weekend} show standard deviation of our measurements,
which is 14.62 \% for weekend data and 15.58 \% for not including weekend data.
Even though the standard deviation is lower when not including weekend data for train data, it's higher for test data, which was not expected.
...
Visual analysis of Figure \ref{fig:ec_histogram_week_only} reveals that the general distribution did not change as compared to Figure \ref{fig:ec_histogram}.
...
The difference in performance can be attributed to changed user behavior after the model was trained.
