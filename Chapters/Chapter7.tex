...
part of the day. The higher the metric the higher the routine. Since routine is detected based on the usage
of appliances it cannot be picked up during the night. It is possible to see that the routine is quite high 
during the morning and evening hours. The anomaly detection algorithm will work best when the metric above is low.
...
This means that the higher the routine, the higher the chance that this bucket will be usable for anomaly detection.
...
But as we can see on figure \ref{fig:ignored_buckets_22}, the high routine rate does not necessarily mean the buckets are useful.
...
We have seen, that anomaly can be detected throughout the day,
but is hard to detect through the night, since appliances are off.
...
To implement this, we would have to build two models.
...
After combining results from all 25 buildings, the following table \ref{tab:ec_res} can be populated.
...
\begin{table}[H]
    ...
    \begin{tabular}[c]{@{}l@{}}Removed \\ min/max\\ outliers\end{tabular} &
      ...
      \multicolumn{1}{l|}{train} &
      test \\ \hline
    0 & \multicolumn{1}{l|}{84.73} & 77.35 & \multicolumn{1}{l|}{86.2} & 78.07 \\ \hline
...
Results show that the algorithm would, on average, label 78 \% of samples as false positive, meaning
...
Similarly, we can check how test data changes over the months. In this case,
