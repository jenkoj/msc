...
In the first part of this Chapter, we will review the existing literature on the topic of LPs and present possible use-cases.
This review will provide a foundation for the subsequent discussion of commonly used LP features in the second part of the Chapter.
Using these features, we will construct a table of profiles, populated with information from the publications reviewed in the first part of the Chapter.
This table will provide an overview of the existing work and allow us to identify any potential gaps in scientific research.
...
One of the first publications on load profiling was published by authors \cite{TRAIN19851103}.
...
\cite{GERBEC2005} tried to assign typical LPs to a particular group of consumers based on their activity. 
...
Another clustering methodology was proposed by authors \cite{Park2019}, using load image profiles and image processing.
...
\cite{Joana2012} clustered different LPs using electricity consumption data and surveys using data from residential homes.
...
One other way to use user LPs is to optimally distribute the load by studying user's usage patterns as \cite{Chuan2014} and \cite{shift2015} proposed in their papers. 
...
Joint actions between them would benefit both as authors show in paper \cite{cooperation2008} and \cite{cooperation2010}
...
The profiles and figure graphics used in Table \ref{fig:map_fig} were sourced from Section \ref{sec:LP_types}.
